...
public class FlutterBranchSdkPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    ...
    var launchOptionsStored: [AnyHashable: Any] = [:]
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        methodChannel = FlutterMethodChannel(
            name: MESSAGE_CHANNEL, binaryMessenger: registrar.messenger())
    ...
    public func application(
        ...
        let disable_nativelink: Bool = {
            guard
                let value = Bundle.main.object(forInfoDictionaryKey: "branch_disable_nativelink")
                    as? Bool
            else {
                return false
            }
        ...
        self.launchOptionsStored = launchOptions
        return true
    }
    ...
    public func application(
        _ app: UIApplication, open url: URL, sourceApplication: String, annotation: Any
    ) -> Bool {
        let branchHandled = Branch.getInstance().application(
            app, open: url, sourceApplication: sourceApplication, annotation: annotation)
        return branchHandled
    }
    ...
    public func onListen(
        ...
        if initialParams != nil {
            ...
            self.eventSink!(
                FlutterError(
                    code: String(self.initialError!.code),
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "setBranchKey":
            if let args = call.arguments as? [String: Any],
                ...
                !branchKey.isEmpty
            {
                Branch.setBranchKey(branchKey)  // <-- change this line
            }
            result(nil)
        case "init":
    ...
    private func setupBranch(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        if !branchAttributionLevel.isEmpty {
            Branch.getInstance().setConsumerProtectionAttributionLevel(
                BranchAttributionLevel(rawValue: branchAttributionLevel))
        }
        ...
        if !snapParameters.isEmpty {
            for param in snapParameters {
                Branch.getInstance().addSnapPartnerParameter(
                    withName: param.key, value: param.value)
            }
        }
        if !facebookParameters.isEmpty {
            for param in facebookParameters {
                Branch.getInstance().addFacebookPartnerParameter(
                    withName: param.key, value: param.value)
            }
        }
        isInitialized = true
        self.startBranchSession()
        result(true)
    }

    private func startBranchSession() {
        guard isInitialized else { return }
        Branch.getInstance().initSession(launchOptions: launchOptionsStored) { (params, error) in
            if error == nil {
                print("Branch InitSession params: \(String(describing: params as? [String: Any]))")
                guard self.eventSink != nil else {
            ...
            if (error == nil && url != nil) || (error != nil && url != nil) {
                ...
                if let err = (error as NSError?) {
                    ...
                    response["errorMessage"] =
                        "errorMessage not returned by Branch SDK. See log for details."
                }
            }
    ...
    private func showShareSheet(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        let response: NSMutableDictionary! = [:]
        buo?.showShareSheet(with: lp, andShareText: shareText, from: controller) {
    ...
    private func getLastAttributedTouchData(
        ...
        Branch.getInstance().lastAttributedTouchData(withAttributionWindow: attributionWindow) {
            latd, error in
            if error == nil {
                if latd != nil {
                    data["latd"] = [
                        "attibution_window": latd!.attributionWindow,
    ...
    private func setDMAParamsForEEA(call: FlutterMethodCall) {
        ...
        DispatchQueue.main.async {
            Branch.setDMAParamsForEEA(
                eeaRegion, adPersonalizationConsent: adPersonalizationConsent,
